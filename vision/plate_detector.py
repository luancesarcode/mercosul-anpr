...
class PlateDetector:
    ...
    def _make_candidate(
        ...
        bbox_global = [crop_bbox[0] + offset[0], crop_bbox[1] + offset[1], crop_bbox[2] + offset[0], crop_bbox[3] + offset[1]]
        candidate = {
            "bbox": bbox_global,
            "det_conf": det_conf,
    ...
    def _evict_stale_cache(self, frame_idx: int) -> None:
        if self._roi_cache_ttl <= 0:
            ...
            return
        stale = [track_id for track_id, entry in self._roi_cache.items() if (frame_idx - entry.frame_idx) > self._roi_cache_ttl]
