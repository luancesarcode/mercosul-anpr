...
from ..ocr_style import inferir_padrao_visual
from .processar_imagem import corrigir_inclinacao_faixa_azul, gerar_variantes_ocr
from .utils import (
...
def _build_candidate_result(candidate_index: int, item):
    ...
    preferred_pattern = inferir_padrao_visual(placa_recortada)

    imagens_para_ocr = gerar_variantes_ocr(placa_recortada, placa_proc)
    for factor, label in ((0.70, "blue-deskew-soft"), (1.0, "blue-deskew-full")):
        corrected = corrigir_inclinacao_faixa_azul(placa_recortada, factor=factor)
        if corrected is None:
            continue
        for variant_name, variant_image in gerar_variantes_ocr(corrected)[:2]:
            imagens_para_ocr.append((f"{label}-{variant_name}", variant_image))
    imagens_para_ocr.append(("deskew", deskew(imagens_para_ocr[-1][1])))
    imagens_para_ocr = imagens_para_ocr[:OCR_MAX_VARIANTS]
    ...
    for origem, imagem_ocr in imagens_para_ocr:
        ...
        for lang in PADDLE_LANGS:
            ...
            candidato = extrair_candidato(texto, score, origem, lang, preferred_pattern)
            if candidato:
                candidatos_validos.append(candidato)
                if (
                    candidato["score"] >= OCR_EARLY_STOP_SCORE
                    and candidato.get("normalization_changes", 0) == 0
                    and candidato.get("trimmed_chars", 0) == 0
                ):
                    early_stop = True
                    break
        if early_stop:
