...
from render.overlay import desenhar_anotacoes, desenhar_debugger_superior_esquerda
...
class PipelineProcessor:
    ...
    def process_frame(self, frame: Any, frame_idx: int, confirmar_veiculo_imediato: bool) -> ProcessedFrame:
        ...
        candidates = self._detect_plate_candidates(frame, vehicles_for_use, frame_idx)
    ...
    def _detect_plate_candidates(
        ...
        if use_roi:
            candidates = self.plate_detector.detect_by_vehicle_rois(frame, vehicles, frame_idx, self.device)
            if candidates:
                return candidates
        return self.plate_detector.detect_global(frame, self.device)
    ...
    def _resolve_plate_predictions(
        ...
        for candidate in candidates:
            ...
            track_id = int(vehicles[vehicle_idx].get("track_id"))
            ...
            if not self._should_display_plate(voted_text, float(voted_score), voted_pattern):
                continue
            smoothed_bbox = self._smooth_bbox(track_id, bbox, frame_idx)
            plates.append(
                {
    ...
    def _get_voted_ocr_for_track(
        ...
        if not self._should_run_ocr(track_id, frame_idx):
