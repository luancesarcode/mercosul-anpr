...
class ProcessingService:
    ...
    def __init__(self, config: AppConfig) -> None:
        ...
        self._device_preference = self._initial_device_preference()
        self._compute_capabilities: dict[str, Any] | None = None
    ...
    @staticmethod
    def _initial_device_preference() -> str:
        from mercosul_anpr.application.compute import normalize_compute_preference

        return normalize_compute_preference(os.getenv("ANPR_COMPUTE_DEVICE", "auto"))

    def compute_status(self, *, refresh: bool = False) -> dict[str, Any]:
        """Return the active preference and a tested NVIDIA capability report."""
        ...
        with self._settings_lock:
            if refresh or self._compute_capabilities is None:
                self._compute_capabilities = inspect_compute_capabilities()
            capabilities = dict(self._compute_capabilities)
            try:
                resolved = resolve_compute_device(self._device_preference, capabilities)
            ...
            return {
                ...
                "resolved_device": "nvidia" if resolved != "cpu" else "cpu",
                "resolved_label": device_label(resolved),
                "busy": self._run_lock.locked(),
                "resolution_error": resolution_error,
                "nvidia": capabilities,
            }

    def set_compute_preference(self, preference: str) -> dict[str, Any]:
        """Change the device for subsequent work without interrupting active inference."""
        from mercosul_anpr.application.compute import normalize_compute_preference, resolve_compute_device

        normalized = normalize_compute_preference(preference, strict=True)
        if not self._run_lock.acquire(blocking=False):
            raise RuntimeError("Aguarde o processamento atual terminar antes de trocar o dispositivo.")
        try:
            with self._settings_lock:
                status = self.compute_status(refresh=normalized == "nvidia")
                resolve_compute_device(normalized, status["nvidia"])
                self._device_preference = normalized
                self._device = None
        finally:
    ...
    def _build_processor(self, config: AppConfig) -> PipelineProcessor:
        from mercosul_anpr.application.compute import resolve_compute_device
        from mercosul_anpr.pipeline.processor import PipelineProcessor
        ...
        if self._device is None:
            ...
            self._device = resolve_compute_device(self._device_preference, status["nvidia"])
